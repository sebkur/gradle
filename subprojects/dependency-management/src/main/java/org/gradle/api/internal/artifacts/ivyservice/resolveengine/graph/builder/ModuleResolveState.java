...
class ModuleResolveState implements CandidateModule {
    ...
    private boolean maybeSkipSelectionChange(ComponentState newSelected) {
        if (selectionChangedCounter == MAX_SELECTION_CHANGE + 1) {
            LOGGER.warn("Version computation for module '{}' changed selection too many times.\n" +
                "Selection will keep highest version to exit resolution for graphs that have no stable output.\n" +
                "Features available in a stable graph are not guaranteed, such as alignment.", id);
        }
